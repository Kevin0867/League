...
/// Flat league leaderboard over the explicit league roster (LeagueTeam), not
/// grouped by division. This is the ladder for the redesigned league: whichever
/// published teams an admin has added play a single round-robin and share one
/// leaderboard. Teams with no games yet appear at the bottom, 0 across.
export async function leagueStandingsFlat(seasonId: string): Promise<LeagueStandingRow[]> {
  ...
  return entries
    .map((e) => ({
      ...(ranked.get(e.team.id) ?? {
        teamId: e.team.id, played: 0, matchesWon: 0, matchesLost: 0, linesWon: 0,
